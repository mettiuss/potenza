...
export async function handleFeedback(interaction: ButtonInteraction) {
    ...
    const message = await interaction.user.send(`<:FNIT_Vindertech:678655323115880512> *Grazie per il tuo feedback!*`);

    // Aggiorna MongoDB
    interaction.client.mongo.feedback.updateOne({"_id": vindertech_id}, like ? {$inc: {"like": 1}}: {$inc: {"nolike": 1}}, { upsert: true })
}
